function intra_train_veinattnet(datasets, seeds, partitionSplit, csvRoot)
    addpath(genpath(pwd));
    ...
    if nargin < 4 || isempty(csvRoot)
        csvRoot = "./tmp/intra_dataset_csv";
    end
    ...
    for di = 1:numel(datasets)
        ...
        for si = 1:numel(seeds)
            ...
            trainCsv = fullfile(csvRoot, dataset, string(statSeed), "train.csv");
            validationCsv = fullfile(csvRoot, dataset, string(statSeed), "validation.csv");
            outputDir = sprintf("./features/%s", modelName);

            fprintf("Running intra VeinAttNet dataset=%s stat_seed=%d\n", dataset, statSeed);
            fprintf("trainCsv=%s\n", trainCsv);
            fprintf("validationCsv=%s\n", validationCsv);
...
function leftOutIds = get_left_out_subject_ids(trainDir, partitionSplit)
    ...
    names = string({classDirs.name});
    keepMask = names ~= "." & names ~= "..";
    names = sort(names(keepMask));
    subjectIds = cellstr(names);
