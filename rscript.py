...
def test():
    pred = torch.tensor([[-1, -1, 1, -1], [-1, 1, -1, -1]]).float()
    x = torch.pi - torch.acos(pred)
    y = torch.tensor([2, 1])
    print(x.shape, y.shape)
    res1 = F.cross_entropy(x, y, reduction="mean")
    res2 = F.cross_entropy(pred, y, reduction="mean")
    print(res1, res2)
        ...
        for method, seeds in methods.items():
            if method == "veinAttNet":
                for seed, run_name in seeds.items():
                    logfile = rf"tmp/{dataset}/{seed}/eval_{dataset}.log"
                    logdir = rf"tmp/{dataset}/{seed}/{dataset}"
                    os.system(f"mv {logdir} tmp/{run_name}/")
                    os.system(f"mv {logfile} tmp/{run_name}/")
                    os.system(f"rm -r tmp/{dataset}/{seed}")
