...
class BalancedIntraWrapper(IntraWrapper):
    ...
    def loop_splitset(self, ssplit: str) -> List[Any]:
        ...
        datalist: List[Any] = []
        sids = list(enumerate(data.keys()))
        random.shuffle(sids)

        for _ in range(2):
            for start in range(0, len(sids), P):
                for offset in range(P):
                    cid, class_name = sids[(start + offset) % len(sids)]
                    images = data[class_name]
                    if len(images) < K:
                        k = 0
                        while k < K:
                            datalist.append((images[k % len(images)], cid))
                            k += 1
                    else:
                        for x in random.sample(images, K):
                            datalist.append((x, cid))
