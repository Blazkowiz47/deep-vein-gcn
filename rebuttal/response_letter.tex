...
\begin{itemize}
    ...
    \item Improved reproducibility by correcting the public repository link and providing the implementation, training and evaluation scripts, model definitions, and trained checkpoints required to reproduce the reported results. \textbf{(see Main Paper: Section 1)}
    
    \item Clarified the experimental repetition strategy by reporting that results are computed over five identity-disjoint statistical splits and are presented as mean values with 95\% confidence intervals. The train--validation partitioning strategy and corresponding image statistics are also reported. \textbf{(see Main Paper: Section 3.2; Supplementary Material: Section 5)}
...
Second, we have revised the experimental section to clearly distinguish between cross-dataset generalisation and enrollment-based unknown-rejection evaluation. We now report results under both full-subject verification and stricter open-set unknown-rejection protocols, including AUC, EER, and TAR at fixed FAR operating points. We have also added identity-disjoint five-fold evaluation with 95\% confidence intervals to improve statistical reliability.
...
\begin{itemize}
	...
	The repository includes the training and evaluation pipeline, model definitions, scripts required to reproduce the reported results, and trained checkpoints. We have also updated the manuscript to include the correct repository link.
\end{itemize}
\color{blue} 
%%%&&&&&&
%%%&&&&&&
...
\begin{itemize}
	...
	\item \textbf{Authors' Action:} 
	We have clarified this point in the revised manuscript \textbf{(please refer Section 3.2, last paragraph; Page 8)}. All reported results are obtained using {five independent runs}, corresponding to an identity-disjoint 5-fold cross-validation strategy on the training data. In each fold, four subsets are used for training and one for validation, resulting in five trained models and five corresponding evaluations on the held-out test dataset.

	The final performance is reported as the {mean and 95\% confidence interval over these five runs}. This clarification has been explicitly added to the experimental protocol section to improve transparency and reproducibility.
\end{itemize}
\color{blue} 
%%%&&&&&&
%%%&&&&&&
...
\begin{itemize}
	\item \textbf{Authors' Response:} 
	We thank the reviewer for pointing out this missing protocol detail. We agree that the train--validation partitioning strategy should be explicitly reported, as it affects reproducibility and the interpretation of the training protocol.

	In the revised manuscript and supplementary material, we clarify that the train--validation split is performed at the \textbf{subject/identity level}, not by randomly splitting images within the same identity. For each statistical split, identities from the source datasets are partitioned into disjoint training and validation subsets. All images belonging to identities assigned to the training subset are used only for training, while all images belonging to identities assigned to the validation subset are used only for validation. Thus, no subject or finger identity appears in both the training and validation sets.

	The image counts reported in the supplementary material are obtained after this subject/identity-disjoint partitioning by counting the images associated with the identities assigned to each subset. These counts are included to clarify the amount of data used for training and validation, as requested by the reviewer, but they do not imply image-level splitting within the same identity.

	\item \textbf{Authors' Action:} 
	We have clarified the train--validation partitioning strategy in the revised manuscript \textbf{(please refer Section 3.2, last paragraph; Page 8)} and provide the corresponding per-dataset train--validation statistics in the supplementary material \textbf{(please refer Section 5 of the Supplementary Material; Page 5)}. The revised description explicitly states that:
	\begin{itemize}
		\item the split is performed at the subject/identity level;
		\item no subject or finger identity is shared between training and validation;
		\item all images belonging to a training identity are used only for training;
		\item all images belonging to a validation identity are used only for validation; and
		\item the reported image counts are obtained after subject/identity-disjoint partitioning.
	\end{itemize}

	This clarification ensures that the training and validation protocol is transparent, reproducible, and free from identity leakage.
\end{itemize}
%%%&&&&&&
\color{blue} 
\textbf{\textit{Comment no. \showReviewcounter:}} 
...
\begin{itemize}
	...
	To address this concern, we have redesigned and clarified the evaluation protocol in the revised manuscript. We now report two complementary protocols. The first is an \textbf{enrollment-based unknown-rejection protocol}, where only 50\% of the subjects from the held-out test dataset are enrolled, while the remaining 50\% are treated as unknown identities. During inference, a probe is accepted only if its maximum cosine similarity score with the enrolled gallery exceeds the decision threshold; otherwise, it is rejected as unknown. This explicitly evaluates the ability of the system to reject non-enrolled identities. The second protocol is a \textbf{full-subject verification protocol}, where all subjects from the held-out test dataset are enrolled, corresponding to subject-disjoint cross-dataset verification.
...
\begin{itemize}
	...
	Following the reviewer’s suggestion, we have now included {intra-database open-set verification experiments} for three datasets (FV-300, FV-USM, and MMCBNU) in \textbf{Section 2 of supplementary material.} In this protocol, each dataset is split into 50\% training, 30\% validation, and 20\% testing subjects, and open-set evaluation is performed by enrolling a subset of identities and treating the remaining identities as unknown. The results show that the proposed method achieves lower EER and higher TAR across most operating points under intra-database conditions, achieving very strong performance on FV-300 and strong performance on FV-USM and MMCBNU.
    

	We note that intra-database experiments were not conducted on PolyU and VERA due to their limited number of samples per identity (4 and 2, respectively), which makes it infeasible to construct meaningful training/validation/test splits for learning-based methods.
	...
	\begin{itemize}
	    ...
	    \item A clear description of the intra-database protocol (50\% train, 30\% validation, 20\% test split).
