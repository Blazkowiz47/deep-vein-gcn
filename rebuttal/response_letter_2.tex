...
\begin{itemize}
    \item Clarified the public availability of FV-300 and added explicit dataset-access information to the revised manuscript and public repository.
...
\begin{itemize}
    \item \textbf{Authors' Response:}
    We thank the reviewer for the positive assessment and this useful suggestion. We agree that fully open checkpoint distribution could further improve reproducibility. The implementation, training and evaluation scripts, and model definitions remain publicly available, while the trained checkpoints can currently be obtained through the access procedure described in the project repository. We have noted the reviewer's suggestion and will consider fully open checkpoint distribution in a future release.

    \item \textbf{Authors' Action:}
    No manuscript change was required for this optional suggestion; the current checkpoint-access information remains available in the project repository.
\end{itemize}
...
\begin{itemize}
    ...
    To resolve this issue, we have now made the FV-300 dataset openly available and have added explicit access information to the revised manuscript and the public project repository. Researchers can therefore obtain the dataset required to reproduce the FV-300 experiments rather than being limited to the four smaller databases. We have also revised the dataset description to distinguish the original source publication from the new public access route provided with this work.

    \item \textbf{Authors' Action:}
    We added the FV-300 access information to the dataset description in the revised manuscript and to the repository README, thereby making the data source for all five evaluated datasets explicit and reproducible.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Authors' Action:}
    After reviewing the organisation in light of the comment, we retained Sections 3--6 as separate sections because their scopes are distinct and their present ordering reflects the intended progression of the paper.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Authors' Action:}
    We added MCP and RLT rows to the intra-database result table for all three evaluated databases and expanded the accompanying discussion to compare within-database FV-300 performance against the $BCDE \rightarrow A$ cross-dataset result.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Authors' Action:}
    We added a concise explanation to the DSConv subsection that distinguishes its continuity-preserving sampling trajectory from more general offset adaptation and directional alignment, and we revised the surrounding claims to avoid overstatement.
\end{itemize}
